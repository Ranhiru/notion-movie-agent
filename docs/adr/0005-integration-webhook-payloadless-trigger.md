...
New Titles are signalled via a **Notion integration webhook** (created in the
...
- **Security trade-off accepted.** The endpoint is unauthenticated, but the blast radius
  ...
  enrichment providers run only for `pending` Titles — which an attacker cannot create
